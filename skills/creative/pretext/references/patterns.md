  ...
  while (y < H - 40) {
    ...
    if (inBand) {
      ...
      if (leftW >= rightW) {
        ...
        x = obstacle.x + half + 12
        w = rightW - 12
      }
      if (w < 40) {
        y += LINE_H
        continue
      } // skip rather than squeeze
    }
...
function pushMorphedRows(aRows, bRows, mix) {
  for (let row = 0; row < rows; row++) {
    ...
    obstacleRows[row].push([a[0] + (b[0] - a[0]) * mix, a[1] + (b[1] - a[1]) * mix])
  }
}
```
...
For a demo that _visualizes_ this, render the card shrinking from `MAX_W` down to `maxLineWidth` over a second — the line count stays constant but the right edge pulls in.
