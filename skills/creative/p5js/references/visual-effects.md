...
function curlNoise(x, y, scale, time) {
  ...
  let dndx =
    (noise(x * scale + eps, y * scale, time) - noise(x * scale - eps, y * scale, time)) / (2 * eps)
  let dndy =
    (noise(x * scale, y * scale + eps, time) - noise(x * scale, y * scale - eps, time)) / (2 * eps)
...
function halftone(sourceBuffer, dotSpacing, maxDotSize) {
  ...
  for (let y = 0; y < height; y += dotSpacing) {
    for (let x = 0; x < width; x += dotSpacing) {
      let idx = 4 * (y * width + x)
      let brightness =
        (sourceBuffer.pixels[idx] + sourceBuffer.pixels[idx + 1] + sourceBuffer.pixels[idx + 2]) / 3
...
function packStep() {
  ...
  for (let attempts = 0; attempts < 100; attempts++) {
    ...
    if (valid) {
      circles.push(new PackedCircle(x, y, 1))
      break
    }
  }
      ...
      for (let dy = -2; dy <= 2; dy++) {
        for (let dx = -2; dx <= 2; dx++) {
          ...
          if (nc >= 0 && nc < cols && nr >= 0 && nr < rows) {
            let gi = nc + nr * cols
            if (grid[gi] !== -1 && points[grid[gi]].dist(sample) < r) {
              ok = false
            }
          }
        }
