# Dreams (Colorlib Dreams) — Tasks & Design Notes

> Prep artifacts prepared on `main` while the Lagoon PR awaits review.
> Implementation ships later on `feat/template-dreams`.
...
- **Design tokens:** brand gold **#f3bd00** (primary buttons/accents) + dark
  ...
  pricing.
- **Recreation name:** Dreams. App folder `apps/dreams`, package
  `@free-react-templates/dreams`.
- **Design approach:** light theme with gold/navy accents; hero = seeded
  ...
  picsum-seeded (`picsum.photos/seed/dreams-N/w/h`); Google Fonts via
  `<link>`.
...
- [x] Write `openspec/specs/template-dreams/spec.md` (Gherkin requirements +
      scenarios + replication findings) — DONE on main (this prep).
- [ ] Validate spec: `npm run spec:validate`.
- [ ] Create `apps/dreams` (copy `apps/aurora`; rename package to
      `@free-react-templates/dreams`).
