# Vitalis (Colorlib Aesthetic) — Tasks & Design Notes

> Prep artifacts prepared on `main` while the Cove PR awaits review.
> Implementation ships later on `feat/template-vitalis`.
...
- **Structure observed (1:1):** navbar (Home/About/Services/Pages/Pricing/
  ...
  ("Find Best Doctors With VITALIS": Advanced equipment, Qualified
...
- **Recreation name:** Vitalis. App folder `apps/vitalis`, package
  `@free-react-templates/vitalis`.
...
- [x] Write `openspec/specs/template-vitalis/spec.md` (Gherkin
      requirements + scenarios + replication findings) — DONE on main (this
      prep).
- [ ] Validate spec: `npm run spec:validate`.
- [ ] Create `apps/vitalis` (copy `apps/aurora`; rename package to
      `@free-react-templates/vitalis`).
