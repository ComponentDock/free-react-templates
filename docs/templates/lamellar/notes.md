# Lamellar — Implementation Tasks & Design Notes

## Source Mapping

- **ColorLib source**: Accordion 06 (`https://colorlib.com/wp/template/accordion-06/`)
- **Preview URL**: `https://preview.colorlib.com/theme/accordion-06/` (404 — screenshot only)
- **New template name**: `lamellar` (single lowercase word, evokes thin layers/lamellae)
- **App folder**: `apps/lamellar`
- **Package**: `@free-react-templates/lamellar`
- **Deploy target**: `https://lamellar.free.componentdock.com/`

## Structure Order (matching screenshot 1:1)

1. **Page wrapper** — dark background (gray-950), min-h-screen, flex column, center content
2. **Heading** — "Accordion #06", centered, dark gray text (gray-900), mb-12
...
- **Text**: "Accordion #06" (exact from screenshot)
...
- [x] Spec written at `openspec/specs/template-lamellar/spec.md`
- [ ] Create app scaffold: `apps/lamellar/` (copy from simplest existing app like `apps/abode`)
- [ ] Update `package.json` name to `@free-react-templates/lamellar`
- [ ] Add `public/CNAME` with `lamellar.free.componentdock.com`
- [ ] Set `homepage` in package.json to `https://lamellar.free.componentdock.com`
...
- [ ] Run `scripts/verify-app.sh lamellar`
- [ ] Fix any failures
