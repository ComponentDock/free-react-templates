...
Prose lives in `apps/quill` and uses shared components from `packages/ui`
...
The system SHALL render a top navigation bar with the site name "Prose",
...
- **WHEN** the page loads
- **THEN** the navbar SHALL show the site name "Prose" and links Home,
...
- [ ] `npm run spec:validate` passes for `template-prose`
