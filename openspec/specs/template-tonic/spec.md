# Template: Tonic (Fitness / Gym Template)

## Purpose

Tonic is a single-page fitness-club / personal-training template in the
free-react-templates monorepo. It is an original React recreation of the
ColorLib "Slim" design (see TEMPLATES.md — Bootstrap category, line 497; the
same source is DUPLICATED at lines 1875 and 2358 — ALL THREE rows must be
marked `[x]` when done; note that "Slim2" at lines 498/1167/1876 is a
DIFFERENT ColorLib template, `wp/template/slim2/`, not this one), built under
...
Testing); a light **Fitness Program** grid of 8 icon cards; a white
**services** block ("Get a Perfect Body" + 5 icon services); a **Training
Schedule** with 7 day tabs and trainer cards; an image-background **counter
band** ("We're Functioning for Almost N Years"); a **testimonials** carousel;
an **Our Coaches** row of 4 circular-photo trainer cards; a light **blog**
grid; a **contact** section (info list + message form) with a full-width
**map**; a **gallery** heading with a photo strip; and a **black footer** with
four columns + social circles. The brand colors are **coral `#fe9191`**
(primary buttons, icons, accents) and its darker shade **`#fe7878`** (promo
band, hover states), on **white** sections with **black `#000000`** navbar /
footer and **`#f8f9fa`** light bands. Font: **Nunito Sans**. Tonic recreates
...
- **Original:** ColorLib "Slim" — `https://colorlib.com/wp/template/slim/`
  (Fitness / Gym Template). TEMPLATES.md has **THREE copies** of this item
  (lines 497, 1875, 2358 — all `- [ ]`); mark ALL THREE `[x]` when done. None
  is shipped yet and no other spec targets this source (verified: no spec
  folder references `wp/template/slim/`; the `template-wisp` folder is for a
  different source).
- **Preview URL:** `https://preview.colorlib.com/theme/slim/` — **HTTP 200**
  (72.8 KB), fully reachable; structure + tokens below are from this live
  DOM, its `css/style.css` (83 KB: bootstrap 4 + custom ftco rules), live
  computed styles verified in a browser, and the TEMPLATES.md screenshot.
  ...
  `index.html`.
- **Screenshot:** `slim-free-template.jpg` (1200×946, verified via browser
  vision): split hero — left side stark white with the dark headline "Get A
  Perfect Body Figure" (light-weight), small coral "WELCOME TO THE CLUB"
  eyebrow, body text and a salmon pill "Join with us" button; right side a
  photo of a woman in teal leggings walking up outdoor stairs; a huge
  vertical **"STRENGTH"** word and a thin coral rectangle outline as graphic
  accents; navbar links top-right with the active link underlined coral. The
  bottom of the screenshot shows the **coral promo band** with three
  white-icon cards (trophy/medal, clipboard-check, sneaker) — "Free Lesson",
  "35% Discount", "Free Testing" — with white titles and small white text.
...
- **Section structure (from the live DOM, in order):**
  1. `nav.navbar.navbar-expand-lg.navbar-dark.ftco_navbar.bg-dark` — **dark
     ...
     (recreate "Tonic") + 8 links: **Home** (active), **Programs**,
  ...
  3. `section.ftco-section.ftco-services.ftco-no-pt.ftco-no-pb` — **coral
     ...
     icon (trophy/medal, clipboard-check, sneaker), `h3` title (white) and a
     short paragraph, plus a **"Read more →"** link. Titles: **Free
     Lesson**, **35% Discount**, **Free Testing**.
  4. `section#programs-section.ftco-section.ftco-no-pb.ftco-no-pt.ftco-program.bg-light`
     — **Fitness Program** grid (bg `#f8f9fa`): centered heading + intro
     paragraph, then **8 program cards** (2 rows × 4) on white cards: each a
     coral flaticon icon (gym, body, woman, abs, running, meditation,
     aerobic, gym) + `h2` title (18–20px black) + short lorem text +
     **"Learn More"** link (coral). Cards: **Fit & Healthy**, **Muscle
     Building**, **Bikini & Body**, **Cardio Exercise**, **Power Yoga**,
     **Aerobics Program**, **Crossfit Program**, **Fitness Program**.
  5. `section#services-section.ftco-section.ftco-services-2` — **"Get a
     ...
     services** (icon + `h3` + text): **Make Your Body Harmonic**, **Weight
     Loss Program**, **Group Personal Trainings**, **Optimal Diet
     Selection**, **Individual Training Programs**.
  6. `section#schedule-section.ftco-section.ftco-schedule` — **Training
     Schedule**: centered subheading + `h2` **"Training Schedule"** + intro.
     Left: **7 day tabs** (`nav-pills`): **Monday Fitness Program / Tuesday
     Crossfit Program / Wednesday Aerobic Program / Thursday Yoga Classes /
     Friday Fitness Program / Saturday Yoga Classes / Sunday Crossfit
     Program**. Right (`div.text.pl-md-5`): **schedule cards** — `time`
     **"08:00AM - 10:00AM"**, `h3` class title (**Basic Exercise**, **Yoga
     Program**, **Body Building**, ...), lorem description, and a trainer
     attribution quote `— Anthony Miller, Cardio Expert` / `— Jason Davis`
     with a small circular trainer photo (`trainer-1..4.jpg`).
  ...
  8. `section.ftco-section.testimony-section` — **Testimonials**: centered
     subheading **"Read testimonials"** + `h2` **"What Client Says"**;
     testimonial cards (owl carousel): quote text + name **"Jeff
     Freshman"** + position **"Artist"** + circular photo.
  9. `section#coaches-section.ftco-section` — **Our Coaches**: subheading +
     `h2` **"Our Coaches"**; **4 coach cards** (`.staff`): **200px circular
     photo** (radius 50%; `trainer-1..4.jpg`), `h3` name (24px, weight 400)
     — **Lloyd Wilson**, **Rachel Parker**, **Ian Smith**, **Alicia
     Henderson** — `span.position` (uppercase, coral, letter-spacing 1px;
     e.g. "Body Builder", "Fitness Coach") and a `div.ftco-social.text-center`
  ...
  14. `footer.ftco-footer.ftco-section` — **footer**, computed background
      **`#000000`** (black), white text, `padding: 6em 0`; 4 columns:
      - **About Tonic.** — brand + lorem paragraph + round social icon
...
- **Behavior notes:** navbar gets a solid black background on scroll
  ...
  counters animate on load (jquery.animateNumber); day tabs switch schedule
  cards; all internal links point to `#` (render as non-navigating or
  `href="#"`); the contact form has no backend (preventDefault / client-side
  success message). Section rhythm: standard `ftco-section` padding `6em 0`
  (the services band and map are exceptions with `ftco-no-pt/ftco-no-pb`).
...
span.number` color, `.staff .position` color, service/program icon colors,
  link hovers, active nav link. Darker shade **#fe7878** — `.ftco-services`
  band bg, `.services-2` card bg, hover states.
...
- Program section: bg `#f8f9fa`; white cards; coral 40px icons; `h2` titles
  black 18–20px; "Learn More" coral link.
- Section headings: `span.subheading` uppercase (16px default, coral); `h2`
  40px weight 600 black (28px mobile); `span.number` coral.
- Schedule: day tabs (white bg, coral active state); schedule cards with
  `time` + `h3` title + lorem + trainer quote (`— Name, Role`) + small
  circular trainer photo.
- Counter band: background image (`divider.jpg`) with overlay; white text;
  `h2` with animated coral number span.
- Testimonials: white cards, quote text, name + position, circular photo.
...
- Imagery: all photos are placeholders in the recreation —
  `https://picsum.photos/seed/tonic-<n>/<w>/<h>` (deterministic per
  template): hero slide photos (seeds tonic-hero1 / tonic-hero2 — must be
  fitness/workout subjects), promo icons (lucide, not photos), 8 program
  icons (lucide), schedule trainer thumbs, about image, 4 testimonial faces,
  4 coach headshots (portrait), 3 blog images, 4 gallery images. **Picsum
  seeds are arbitrary — verify the seeds render suitable subjects before
  shipping and pin verified seeds (see `docs/templates/tonic/tasks.md`).**
  Icons from `lucide-react` (dumbbell, heart-pulse, users, calendar, clock,
  map-pin, phone, mail, globe, chevron-right, menu, message-circle, trophy,
...
The system SHALL render the light program section with eight icon cards.
...
- **THEN** the heading "Fitness Program" with an intro paragraph SHALL
  render centered
- **AND** eight program cards SHALL render (Fit & Healthy, Muscle Building,
  Bikini & Body, Cardio Exercise, Power Yoga, Aerobics Program, Crossfit
  Program, Fitness Program), each with a coral icon, a black title, a short
  text and a "Learn More" link
...
The system SHALL render the schedule section with seven day tabs and
trainer cards.
...
- **AND** a schedule card SHALL render with a time ("08:00AM - 10:00AM"), a
  ...
  with a circular photo (e.g. "— Anthony Miller, Cardio Expert")
...
- **THEN** a black footer SHALL render with the brand column ("About Tonic."
...
- **AND** the document title SHALL be "Tonic — Fitness Club"

## Verification checklist

- [ ] `npm run spec:validate` passes.
- [ ] `npm run verify:app -- tonic` passes: typecheck → lint → vitest
      (100% coverage) → build.
- [ ] Layout matches the original 1:1: transparent navbar (scrolled = solid
      ...
      8 program cards on `#f8f9fa`, "Get a Perfect Body" services (5), 7 day
      tabs + schedule cards, image-bg counter band, testimonials, 4 coaches
      with circular photos, 3 blog cards, contact info + form + map, gallery
      strip, black footer with Component Dock credit.
- [ ] Design tokens applied: coral `#fe9191` (buttons, icons, accents),
      `#fe7878` (promo band), Nunito Sans, pill buttons (radius 30px),
      black `#000` navbar/footer, `#f8f9fa` light bands.
- [ ] No ColorLib references in any app file; footer links
      `https://www.componentdock.com/`.
- [ ] TEMPLATES.md rows 497, 1875 and 2358 all marked `[x]` with the surge
      URL after shipping.
