...
```gherkin
Feature: EncoreMix Music Festival Landing Page

  Background:
    Given the user is on the EncoreMix landing page

  Scenario: Page loads with hero visible
    Then the hero section is visible
    And the headline text is displayed with gradient styling
  ...
  Scenario: Mobile hamburger menu
    ...
    Then a slide-in navigation panel appears
    And the panel shows all navigation links

  Scenario: Countdown timer ticks down
    Then the countdown displays the number of days, hours, minutes, seconds
    And the seconds value decrements every second

  Scenario: Headliner artists section displays three artists
    Then the "Lineup Artists — Headliners" section is visible
    And 3 headliner artist cards are displayed
    And each card has a photo, name, bio text, and link

  Scenario: Complete lineup shows artist grid
    Then the "Complete Lineup" section is visible
    And 8 artist thumbnails are displayed in a 4-column grid
  ...
  Scenario: Next events carousel
    Then the "Our Next Events" section is visible
    And event cards are displayed in a horizontal carousel
  ...
  Scenario: Latest news section
    Then the "Our Last News" section is visible
