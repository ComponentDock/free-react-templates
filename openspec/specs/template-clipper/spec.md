# Template: Clipper (Barber Shop Landing)

## Purpose

Clipper is a single-page barber-shop landing template in the free-react-templates
...
Tailwind CSS 4 + TypeScript. The recreation carries the NEW name "Clipper"
...
discount"), a video + "Good Looking Style" split, and a light footer. Clipper
...
- **Live preview DOM analyzed:** `https://preview.colorlib.com/theme/alotan/`
  ...
  accents, photo hero, script wordmark).
- **Section order (1:1):**
  1. Header/navbar (absolute over hero): logo + nav (Home, Haircut, About,
     Contact) + dark-mode toggle.
  2. Hero (`site-hero overlay`, dark photo background, centered): barbershop
     wordmark (script "Clipper" + "Barber Shop" caption, recreated as styled
     text), no headline copy.
  ...
  4. Services (`site-section`): "Services" heading + blurb + Previous/Next
     buttons; photo cards (Haircuting · Beard Shaving · Cream & Shampoo)
     each with an icon, title, and "Read More".
  5. Barber Features (`site-section`): centered "Barber Features" heading +
     blurb + 6 icon feature cards: Shave & Haircut · Cream & Shampoo ·
     Mustache Expert · Haircut Styler · Razor For Beards · Haircomb.
  ...
  7. Good Looking Style (`site-section`): video image with play button +
     ...
     paragraphs.
  8. Footer (`site-footer`, light gray `#f9f9f9`): About column (blurb,
     address, phone, email) + Links column + Latest Blog column (post meta +
     titles) + Connect column (social icons) + copyright bar.
- **Design tokens extracted from `css/style.css`:**
  - Brand color: **#ff6d24** (orange — quick-info strip, accents, hover).
  - Text: dark gray on white; black buttons (`btn-black`).
  - Font: **"Roboto"** (Google Fonts) — body and headings; script wordmark
    recreated with "Dancing Script" (Google Fonts).
  - Buttons: solid black "Make an Appointment"; small prev/next square
...
- **Recreation decisions:** repo-standard Navbar (site name, section links,
  ...
  "Clipper" wordmark; quick-info strip with lucide icons (phone, map pin,
...
Clipper lives in `apps/clipper` and uses shared components from `packages/ui`
...
The system SHALL render a top navigation bar with the site name "Clipper", a
"Home" link, and a dark-mode toggle button.

#### Scenario: Navbar content

- **GIVEN** the Clipper page is rendered
- **WHEN** the page loads
- **THEN** the navbar SHALL show the site name "Clipper" and a "Home" link pointing to the page root
...
- **THEN** it SHALL show a background image and the site wordmark "Clipper"
...
The system SHALL render a "Barber Features" section with six feature cards.
...
- **AND** it SHALL render six feature cards (Shave & Haircut, Cream & Shampoo, Mustache Expert, Haircut Styler, Razor For Beards, Haircomb)
...
- **THEN** it SHALL show the site name "Clipper" and quick links
...
- **GIVEN** the Clipper app is rendered
- **WHEN** the page loads
- **THEN** the page SHALL compose the Navbar in the banner landmark, all sections in the main landmark, and the Footer in the contentinfo landmark
- **AND** the document title SHALL be "Clipper — Barber Shop Template"
