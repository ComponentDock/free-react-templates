# Template: Multichip (Multi-Select Form — Tag-Style Selection)

## Purpose

Multichip is a single-page multi-select FORM snippet in the free-react-templates
...
built under a DIFFERENT name (**Multichip**), with the monorepo stack:
Vite + React 19 + Tailwind CSS 4 + TypeScript.

The original is a Chosen.js jQuery plugin multi-select rendered on a light-grey
page: a centered `col-md-5` container with a `<select multiple>` element
containing 8 tech-category options (Design, HTML5, CSS3, jQuery, BS4, Bootstrap,
WordPress, FrontEnd). Selected items appear as colored "chip" tags inside the
input area — the key visual feature. Three color themes are demonstrated
(default grey, green, blue).
...
| `--chip-default` | `#eee` / `#e5e4cc` | Default chip background |
| `--chip-green` | `#c7f0db` | Green variant chip background |
| `--chip-blue` | `#d3f4ff` | Blue variant chip background |
...
| Border radius (chips) | none (Chosen default, sharp edges on tag chips) | Chip corners |
...
- **GIVEN** the page is loaded with the Multichip multi-select widget
...
- **AND** the heading reads "Multi-Select #9"
...
- **THEN** I see three multi-select widgets arranged vertically
- **AND** the first widget uses default grey chip styling
- **AND** the second widget uses green chip styling (`#c7f0db`)
- **AND** the third widget uses blue chip styling (`#d3f4ff`)

## Verification checklist

- [ ] `openspec/specs/template-multichip/spec.md` exists with all sections
- [ ] `docs/templates/multichip/tasks.md` exists
- [ ] Source mapping recorded: ColorLib "Multiselect 19" → "Multichip"
...
- [ ] `public/CNAME` contains `multichip.free.componentdock.com`
- [ ] `homepage` set to `https://multichip.free.componentdock.com`
