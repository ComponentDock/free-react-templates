import { Check } from 'lucide-react'

interface MentorProps {
  className?: string
}

interface MentorRow {
  image: string
  headline: string
  description: string
  items: string[]
}

const rows: MentorRow[] = [
  {
    image: 'https://picsum.photos/seed/cruiser-5/800/600',
    headline: 'Personal Coaching',
    description:
      'Get personalized mentorship from experienced skaters who have competed at the highest levels.',
    items: [
      'Custom training plans',
      'Progress video analysis',
      'Mental game coaching',
      'Competition prep',
    ],
  },
  {
    image: 'https://picsum.photos/seed/cruiser-6/800/600',
    headline: 'Group Sessions',
    description:
      'Join our group sessions to learn alongside fellow skaters, build community, and push each other forward.',
    items: [
      'Weekly group rides',
      'Skill-level matching',
      'Guest pro skaters',
      'Social skate events',
    ],
  },
]

export function Mentor({ className = '' }: MentorProps) {
  return (
    <section className={`py-20 bg-surface ${className}`}>
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 space-y-16">
        {rows.map((row, idx) => (
          <div
            key={row.headline}
            className={`flex flex-col items-center gap-10 ${
              idx % 2 === 0 ? 'lg:flex-row-reverse' : 'lg:flex-row'
            }`}
          >
            <div className="lg:w-1/2">
              <img
                src={row.image}
                alt={row.headline}
                className="w-full rounded-lg object-cover shadow-lg"
              />
            </div>
            <div className="lg:w-1/2">
              <h2 className="font-oswald text-3xl md:text-4xl font-bold uppercase tracking-wider text-white mb-4">
                {row.headline}
              </h2>
              <p className="text-muted leading-relaxed mb-6">{row.description}</p>
              <ul className="space-y-3">
                {row.items.map((item) => (
                  <li key={item} className="flex items-center gap-3 text-gray-300">
                    <Check size={18} className="text-brand flex-shrink-0" />
                    {item}
                  </li>
                ))}
              </ul>
            </div>
          </div>
        ))}
      </div>
    </section>
  )
}
