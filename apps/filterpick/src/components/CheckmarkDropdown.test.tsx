...
import type { Option } from './CheckmarkDropdown'
...
describe('CheckmarkDropdown', () => {
  it('renders the trigger button with placeholder text', () => {
    render(<CheckmarkDropdown options={OPTIONS} />)
  ...
  it('shows "N items selected" when options are selected', () => {
    render(<CheckmarkDropdown options={OPTIONS} selectedValues={['orange', 'apple']} />)
  ...
  it('shows "1 item selected" when one option is selected', () => {
    render(<CheckmarkDropdown options={OPTIONS} selectedValues={['orange']} />)
  ...
  it('calls onChange when an option is clicked', async () => {
    ...
    render(<CheckmarkDropdown options={OPTIONS} onChange={onChange} />)
    await user.click(screen.getByRole('button', { name: /select options/i }))
  ...
  it('deselects an already selected option', async () => {
    const user = userEvent.setup()
    const onChange = vi.fn()
    render(<CheckmarkDropdown options={OPTIONS} selectedValues={['orange']} onChange={onChange} />)
  ...
  it('shows Clear All button when all are selected', async () => {
    ...
    render(<CheckmarkDropdown options={OPTIONS} selectedValues={enabledValues} />)
  ...
  it('Clear All clears all selections', async () => {
    ...
    const enabledValues = OPTIONS.filter((o) => !o.disabled).map((o) => o.value)
    render(
      <CheckmarkDropdown options={OPTIONS} selectedValues={enabledValues} onChange={onChange} />,
    )
    await user.click(screen.getByRole('button', { name: /5 items selected/i }))
  ...
  it('closes on click outside', async () => {
    ...
    render(
      <div>
        <CheckmarkDropdown options={OPTIONS} />
  ...
  it('uses custom placeholder', () => {
    render(<CheckmarkDropdown options={OPTIONS} placeholder="Pick fruits" />)
