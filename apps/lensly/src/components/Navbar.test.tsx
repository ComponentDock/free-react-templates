...
describe('Navbar', () => {
  it('renders the uppercase brand and desktop links with Home active at the top', () => {
    setScrollY(0)
    render(<Navbar />)
    expect(screen.getByRole('link', { name: /^Lensly$/ })).toBeInTheDocument()
    expect(screen.getByRole('link', { name: /^Lensly$/ }).className).toContain('text-white')
  ...
  it('turns into a fixed white bar with amber brand and black links when scrolled', () => {
    setScrollY(200)
    render(<Navbar />)
    ...
    expect(screen.getByRole('link', { name: /^Lensly$/ }).className).toContain('text-brand')
  ...
  it('removes the scroll listener on unmount', () => {
    ...
    unmount()
    expect(screen.queryByRole('link', { name: /^Lensly$/ })).not.toBeInTheDocument()
  })
  ...
  it('closes the mobile menu when a link is chosen', async () => {
    ...
    const mobilePhotography = screen.getAllByRole('link', { name: 'Photography' }).at(-1)
    await user.click(mobilePhotography!)
    expect(screen.queryByRole('navigation', { name: 'Mobile' })).not.toBeInTheDocument()
  })
})
