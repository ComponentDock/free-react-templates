...
const navLinks = [
  ...
  { label: 'About', href: '#about' },
  {
    ...
    children: [
      ...
      { label: 'Weight Lifting', href: '#classes' },
      { label: 'Boxing', href: '#classes' },
      { label: 'Running', href: '#classes' },
    ],
  },
...
export function Navbar() {
  ...
  return (
    <header className="relative z-50">
      {/* Top bar */}
      <div className="bg-ink text-white text-xs py-2 px-4">
        <div className="mx-auto max-w-7xl flex items-center justify-between">
          ...
          <div className="flex items-center gap-6">
            <span className="hidden sm:inline">info@peakform.com</span>
      ...
      <nav className="bg-white shadow-sm" aria-label="Main navigation">
        <div className="mx-auto max-w-7xl flex items-center justify-between px-4 py-4">
          ...
          {/* Desktop */}
          <ul className="hidden md:flex items-center gap-8 text-sm font-semibold text-ink uppercase">
            {navLinks.map((link) => (
              <li key={link.label} className="relative">
                {link.children ? (
                  ...
                  >
                    ...
                    {dropdownOpen && (
                      <ul className="absolute top-full left-0 mt-2 bg-white shadow-lg rounded-lg py-2 min-w-[180px] z-50">
          ...
          <ul className="md:hidden bg-white border-t px-4 py-4 space-y-3">
            {navLinks.map((link) => (
              <li key={link.label}>
                <a
                  href={link.href}
                  className="block text-sm font-semibold text-ink uppercase hover:text-brand"
                >
                  {link.label}
                </a>
              </li>
