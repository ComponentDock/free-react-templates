...
describe('Navbar', () => {
  ...
  it('shows the wordmark and section links', () => {
    ...
    expect(screen.getByText('Blastoff')).toBeInTheDocument()
  ...
  it('shows the Get Early Access CTA and the dark-mode toggle', () => {
    render(<Navbar />)

    expect(screen.getAllByRole('link', { name: 'Get Early Access' }).length).toBeGreaterThanOrEqual(
      1,
    )
  ...
  it('toggles dark mode, persists it and cleans up on unmount', async () => {
    ...
    expect(window.localStorage.getItem('blastoff-dark')).toBe('1')

    await user.click(toggle)
    expect(document.documentElement.classList.contains('dark')).toBe(false)
    expect(window.localStorage.getItem('blastoff-dark')).toBe('0')
  })

  it('restores the persisted dark preference on load', () => {
    window.localStorage.setItem('blastoff-dark', '1')
    render(<Navbar />)
  ...
  it('closes the menu when a drawer link is clicked', async () => {
    ...
    await user.click(screen.getAllByRole('link', { name: 'Testimonials' })[1]!)
    expect(menuButton).toHaveAttribute('aria-expanded', 'false')
  })
  ...
  it('closes the menu when the drawer CTA is clicked', async () => {
    ...
    await user.click(screen.getAllByRole('link', { name: 'Get Early Access' })[1]!)
    expect(menuButton).toHaveAttribute('aria-expanded', 'false')
  })
