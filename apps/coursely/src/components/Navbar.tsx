...
export function Navbar() {
  ...
  const [dark, setDark] = useState(() => window.localStorage.getItem('coursely-dark') === '1')
  ...
  useEffect(() => {
    ...
    window.localStorage.setItem('coursely-dark', dark ? '1' : '0')
    return () => {
        ...
        <div className="flex flex-col gap-1">
          {links.map((link) => (
            <a
              ...
              className="block rounded-lg py-3 text-base font-medium text-gray-900 transition-colors hover:text-primary-600 dark:text-white"
            >
              {link.label}
            </a>
          ))}
