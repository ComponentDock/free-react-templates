  ...
  return (
    <footer id="contact" className="bg-surface">
      <div className="mx-auto grid max-w-7xl gap-10 px-4 py-14 sm:grid-cols-2 sm:px-6 lg:grid-cols-3">
        <div>
          <h3 className="font-display text-lg font-bold uppercase text-heading">About Us</h3>
          <p className="mt-4 text-sm leading-relaxed text-body-text">
            BodyForge is a premier fitness center dedicated to helping you achieve your health and
            fitness goals. Join our community and transform your life.
          </p>
        </div>

        <div>
          <h3 className="font-display text-lg font-bold uppercase text-heading">Contact Us</h3>
          <ul className="mt-4 space-y-3 text-sm text-body-text">
            <li>123 Fitness Avenue, Gym City</li>
            <li>
              <a href="tel:+18005551234" className="transition-colors hover:text-brand">
                +1 (800) 555-1234
              </a>
            </li>
            <li>
              <a href="mailto:info@bodyforge.com" className="transition-colors hover:text-brand">
                info@bodyforge.com
              </a>
            </li>
          </ul>
        ...
        <div>
          <h3 className="font-display text-lg font-bold uppercase text-heading">Newsletter</h3>
          ...
          <form onSubmit={handleSubmit} className="mt-4 flex gap-2">
            ...
            <input
              ...
              className="flex-1 rounded border border-gray-300 bg-white px-4 py-2 text-sm text-heading focus:border-brand focus:outline-none focus:ring-2 focus:ring-brand/30"
            />
            <button
              type="submit"
              className="flex items-center justify-center rounded bg-brand px-4 py-2 text-white transition-colors hover:bg-brand-dark"
      ...
      <div className="border-t border-gray-300">
        <div className="mx-auto flex max-w-7xl flex-col items-center justify-between gap-4 px-4 py-6 sm:flex-row sm:px-6">
          <p className="text-sm text-body-text">
            © {new Date().getFullYear()} BodyForge. Made with{' '}
            <a
          ...
          <div className="flex gap-4">
            {[
              ...
              { svg: dribbbleSvg, label: 'Dribbble' },
            ].map(({ svg, label }) => (
              <a
                key={label}
                href={`#${label.toLowerCase()}`}
                className="flex h-9 w-9 items-center justify-center rounded-full bg-heading text-white transition-colors hover:bg-brand"
                aria-label={label}
              >
                {svg}
              </a>
