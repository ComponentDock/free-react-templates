...
import { featureItems, servicesEyebrow, servicesLead, servicesTitle, type Feature } from '../data'
...
export function Services() {
  return (
    <section aria-label={servicesTitle} className="bg-[#f8f9fa] py-20">
      <div className="mx-auto max-w-7xl px-4 sm:px-8">
        <div className="mx-auto max-w-2xl text-center">
          <span className="text-xs font-bold uppercase tracking-[0.3em] text-brand">
            {servicesEyebrow}
          </span>
          <h2 className="mt-2 text-4xl font-black text-ink md:text-[2.5rem]">{servicesTitle}</h2>
          <p className="mt-4 text-muted">{servicesLead}</p>
        </div>
        <ul className="mt-14 grid gap-8 sm:grid-cols-2 lg:grid-cols-3">
          {featureItems.map((item) => {
            ...
            return (
              <li
                key={item.title}
                className="group flex flex-col items-center bg-white p-8 text-center transition-colors duration-300 hover:bg-brand"
              >
