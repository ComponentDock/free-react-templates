...
describe('Navbar', () => {
  it('renders the brand, nav links, CTA, and dark-mode toggle in a sticky header', () => {
    ...
    expect(screen.getByRole('link', { name: /sunflux home/i })).toBeInTheDocument()
  ...
  it('toggles dark mode and persists the choice to localStorage', async () => {
    ...
    expect(window.localStorage.getItem('sunflux-dark')).toBe('1')
    await user.click(toggle)
    expect(document.documentElement.classList.contains('dark')).toBe(false)
    expect(window.localStorage.getItem('sunflux-dark')).toBe('0')
  })

  it('restores a previously stored dark preference on mount', () => {
    window.localStorage.setItem('sunflux-dark', '1')
    render(<Navbar />)
    expect(document.documentElement.classList.contains('dark')).toBe(true)
  })
