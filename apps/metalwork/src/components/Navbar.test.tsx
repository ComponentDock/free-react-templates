...
describe('Navbar', () => {
  it('shows the brand, section links, and a search form', () => {
    render(<Navbar />)

    expect(screen.getByRole('link', { name: 'Metalwork' })).toHaveAttribute('href', '#home')
  ...
  it('submits the desktop search form without navigating', () => {
    render(<Navbar />)
  ...
  it('expands and collapses the mobile menu from the hamburger toggle', async () => {
    ...
    expect(screen.getAllByRole('searchbox', { name: 'Search' })).toHaveLength(2)
    ...
    // The mobile search form submits without navigating.
    await user.click(toggle)
