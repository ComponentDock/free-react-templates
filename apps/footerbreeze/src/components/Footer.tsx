    ...
    >
      <div className="mx-auto max-w-6xl">
        <div className="grid grid-cols-1 gap-8 md:grid-cols-4">
          {/* Column 1: Brand */}
          <div>
            <h2 className="mb-4 text-[1.5rem] font-bold text-white">FooterBreeze</h2>
            <p className="text-sm leading-relaxed text-footer-muted">
              Discover a curated collection of modern web templates designed to elevate your digital
              presence with style and simplicity.
            </p>
          </div>
            ...
            <ul className="list-none space-y-[10px]">
              {primaryNav.map((link) => (
                <li key={link.label}>
                  <a
                    href={link.href}
                    className="text-sm text-footer-nav transition-colors hover:text-white"
                  >
                    {link.label}
                  </a>
                </li>
          ...
          <div>
            <ul className="mb-6 list-none space-y-[10px]">
              {legalNav.map((link) => (
                <li key={link.label}>
                  <a
                    href={link.href}
            ...
            <ul className="mb-6 flex list-none gap-3">
              {socials.map((social) => (
                <li key={social.name}>
                  <a
                    href={social.href}
                    ...
                    className="inline-flex h-9 w-9 items-center justify-center text-footer-nav transition-colors hover:text-white"
                  >
                    <social.icon />
                  </a>
                </li>
            ...
            <a
              href="#"
              className="inline-block rounded-[30px] bg-cta-pink px-5 py-[10px] text-sm font-medium text-white transition-opacity hover:opacity-90"
            >
              Contact Us
            </a>
          </div>
        ...
        <div className="mt-12 border-t border-footer-border pt-8 text-center">
          <p className="text-sm text-footer-muted">
            &copy; 2019-2020 All Rights Reserved. Made with{' '}
            <a
              href="https://www.componentdock.com/"
              target="_blank"
              rel="noreferrer"
              className="text-footer-muted underline transition-colors hover:text-white"
            >
              Component Dock
            </a>
          </p>
