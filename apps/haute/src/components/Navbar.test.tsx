...
const navLabels = ['Home', 'the Brand', 'Portfolio', 'Shop', 'Blog', 'Contact']

describe('Navbar', () => {
  it('renders the logo, tagline, nav links, and social icons', () => {
    render(<Navbar />)
    const banner = screen.getByRole('banner')
    expect(banner).toBeInTheDocument()

    const logo = screen.getByRole('link', { name: /haute/i })
    expect(logo).toHaveTextContent('Fashion Forward')

    const nav = screen.getByRole('navigation', { name: 'Primary' })
    for (const label of navLabels) {
      expect(withinNav(nav, label)).toBeInTheDocument()
    }
    for (const label of ['Pinterest', 'Facebook', 'Twitter']) {
  ...
  it('closes the menu from a nav link, the toggle, and a social icon', async () => {
    const user = userEvent.setup()
    render(<Navbar />)
    ...
    await user.click(screen.getByRole('button', { name: 'Open menu' }))
    await user.click(screen.getAllByRole('link', { name: 'Pinterest' })[1]!)
    expect(screen.queryByRole('navigation', { name: 'Mobile' })).not.toBeInTheDocument()
  })
})
