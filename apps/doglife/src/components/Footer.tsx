...
export function Footer() {
  ...
  return (
    <footer className="bg-brand py-16 text-white/50">
          ...
          <form onSubmit={handleSubmit} className="mt-5">
            ...
            <div className="flex items-stretch gap-2">
              <input
                ...
                className="h-11 min-w-0 flex-1 rounded-md border border-white/10 bg-white/5 px-4 text-sm text-white placeholder:text-white/40 focus:border-black focus:outline-none"
              />
              <button
                type="submit"
                className="flex h-11 items-center gap-2 rounded-md bg-black px-5 text-sm font-semibold text-white transition-colors hover:bg-brand-strong"
              >
                Send
      ...
      <div className="mx-auto mt-14 flex max-w-6xl flex-col items-center justify-between gap-6 border-t border-white/10 px-4 pt-8 sm:flex-row sm:px-6">
        <p className="text-sm font-light">
          Copyright © {new Date().getFullYear()} Doglife. All rights reserved.
        </p>
      </div>
    </footer>
  )
