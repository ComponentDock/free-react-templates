 ...
 * Active state uses purple accent (#563d7c) on the border and text.
 */
      ...
      {items.map((item) => {
        ...
        return (
          <div
            key={item.id}
            className={cn(
              'rounded-lg bg-accordion-bg transition-all',
          ...
          >
            <button
              ...
              className={cn(
                ...
                isOpen ? 'text-accent' : 'text-text-body',
              )}
            >
              <ChevronDown
                aria-hidden="true"
              ...
              />
              <h3 className="font-heading text-sm font-normal">{item.question}</h3>
            </button>
            {isOpen && (
              <div
                id={`${item.id}-panel`}
