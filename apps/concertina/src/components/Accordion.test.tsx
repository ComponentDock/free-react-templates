...
describe('Accordion', () => {
  it('renders all five accordion items', () => {
    ...
    expect(screen.getByText('How to update your billing information?')).toBeInTheDocument()
    expect(screen.getByText('How to contact customer support?')).toBeInTheDocument()
  })
  ...
  it('active item shows purple accent border', async () => {
    const user = userEvent.setup()
    render(<Accordion items={ACCORDION_ITEMS} />)
    const button = screen.getByRole('button', { name: 'How to download and register?' })
    await user.click(button)
  ...
  it('expands the fifth item when clicked', async () => {
    const user = userEvent.setup()
    render(<Accordion items={ACCORDION_ITEMS} />)
    const button = screen.getByRole('button', { name: 'How to contact customer support?' })
    await user.click(button)

    expect(button).toHaveAttribute('aria-expanded', 'true')
    expect(
      screen.getByRole('region', { name: 'How to contact customer support?' }),
    ).toBeInTheDocument()
  })
})
