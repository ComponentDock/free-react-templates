...
describe('Navbar', () => {
  it('renders the site name, links, and dark-mode toggle', () => {
    render(<Navbar />)
    expect(screen.getByRole('link', { name: /Terracotta/i })).toBeInTheDocument()
  ...
  it('closes the mobile menu when a mobile link is clicked', async () => {
    ...
    render(<Navbar />)
    await user.click(screen.getByRole('button', { name: /open menu/i }))
    const serviceLinks = screen.getAllByRole('link', { name: /Service/i })
    const mobileLink = serviceLinks[serviceLinks.length - 1]!
