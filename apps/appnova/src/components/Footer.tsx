    ...
    <footer id="contact" className="bg-night py-16 text-gray-300">
      <div className="mx-auto grid max-w-6xl gap-12 px-4 sm:px-6 md:grid-cols-2 lg:grid-cols-6">
        ...
        <div className="lg:col-span-2">
          ...
          <form onSubmit={handleSubmit} className="mt-6 flex items-stretch">
            <label htmlFor="appnova-newsletter" className="sr-only">
              Email address
            </label>
            <input
              id="appnova-newsletter"
              type="email"
              required
              value={email}
              onChange={(event) => setEmail(event.target.value)}
              placeholder="Your Email Address"
              className="h-12 min-w-0 flex-1 border border-white/25 bg-transparent px-5 text-sm text-white placeholder:text-white/50 focus:border-purple-500 focus:outline-none"
            />
            <button
              type="submit"
              aria-label="Subscribe"
              className="flex h-12 w-12 shrink-0 items-center justify-center bg-gradient-to-b from-purple-500 to-purple-600 text-white transition-opacity hover:opacity-90"
            >
      ...
      <div className="mx-auto mt-14 flex max-w-6xl flex-col items-center justify-between gap-6 border-t border-white/10 px-4 pt-8 sm:flex-row sm:px-6">
        <p className="text-sm text-gray-500">
          Copyright © {new Date().getFullYear()} Appnova. All rights reserved.{' '}
          <a
