    ...
    <div className="mt-8 flex items-center justify-center gap-3">
      {socials.map(({ name, href, color, svg }) => (
        <a
          ...
          href={href}
          aria-label={name}
          className={`flex h-10 w-10 items-center justify-center rounded-full border border-white/20 transition-colors hover:border-brand-cyan hover:text-brand-cyan ${color}`}
        >
          <svg
