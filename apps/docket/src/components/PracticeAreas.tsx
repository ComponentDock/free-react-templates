...
export function PracticeAreas() {
  return (
    <section aria-labelledby="practice-heading" className="bg-white py-20">
      <div className="mx-auto max-w-6xl px-4 sm:px-6">
        <h2 id="practice-heading" className="mb-12 text-center text-3xl font-bold text-[#212529]">
          What We Cover
        </h2>
        <div className="grid grid-cols-1 gap-8 sm:grid-cols-2 lg:grid-cols-3">
          {areas.map((area) => {
            ...
            return (
              <div
                key={area.title}
                className="rounded border border-gray-200 p-8 transition-shadow hover:shadow-lg"
              >
                <Icon className="mb-4 h-10 w-10 text-[#007bff]" aria-hidden={true} />
                <h3 className="mb-2 text-lg font-bold text-[#212529]">{area.title}</h3>
                <p className="text-sm leading-relaxed text-[#b2b2b8]">{area.description}</p>
              </div>
            )
          })}
        </div>
