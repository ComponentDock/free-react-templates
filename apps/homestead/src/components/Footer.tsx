import { ChevronRight, Mail, MapPin, Phone } from 'lucide-react'
import {
  aboutUsLinks,
  brandBlurb,
  brandName,
  communityLinks,
  companyLinks,
  footerContact,
} from '../data'

interface SocialLink {
  label: string
  href: string
}

const socialLinks: SocialLink[] = [
  { label: 'Twitter', href: 'https://twitter.com' },
  { label: 'Facebook', href: 'https://facebook.com' },
  { label: 'Instagram', href: 'https://instagram.com' },
]

function BrandIcon({ label }: { label: string }) {
  const common = 'h-5 w-5 fill-current'
  switch (label) {
    case 'Twitter':
      return (
        <svg viewBox="0 0 24 24" aria-hidden="true" className={common}>
          <path d="M23.953 4.57a10 10 0 0 1-2.825.775 4.958 4.958 0 0 0 2.163-2.723c-.951.555-2.005.959-3.127 1.184a4.92 4.92 0 0 0-8.384 4.482C7.69 8.095 4.067 6.13 1.64 3.162a4.822 4.822 0 0 0-.666 2.475c0 1.71.87 3.213 2.188 4.096a4.904 4.904 0 0 1-2.228-.616v.06a4.923 4.923 0 0 0 3.946 4.827 4.996 4.996 0 0 1-2.212.085 4.936 4.936 0 0 0 4.604 3.417 9.867 9.867 0 0 1-6.102 2.105c-.39 0-.779-.023-1.17-.067a13.995 13.995 0 0 0 7.557 2.209c9.053 0 13.998-7.496 13.998-13.985 0-.21 0-.42-.015-.63A9.935 9.935 0 0 0 24 4.59z" />
        </svg>
      )
    case 'Facebook':
      return (
        <svg viewBox="0 0 24 24" aria-hidden="true" className={common}>
          <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z" />
        </svg>
      )
    case 'Instagram':
      return (
        <svg viewBox="0 0 24 24" aria-hidden="true" className={common}>
          <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.052.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98C8.333 23.986 8.741 24 12 24c3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 1 0 0 12.324 6.162 6.162 0 0 0 0-12.324zM12 16a4 4 0 1 1 0-8 4 4 0 0 1 0 8zm6.406-11.845a1.44 1.44 0 1 0 0 2.881 1.44 1.44 0 0 0 0-2.881z" />
        </svg>
      )
  }
}

function FooterLinkColumn({
  title,
  links,
}: {
  title: string
  links: { label: string; href: string }[]
}) {
  return (
    <div>
      <h2 className="mb-10 text-xl font-normal text-ink">{title}</h2>
      <ul className="space-y-3">
        {links.map((link) => (
          <li key={link.label}>
            <a
              href={link.href}
              className="flex items-center gap-1 text-black/80 transition-colors hover:text-brand"
            >
              <ChevronRight className="h-4 w-4 text-brand" aria-hidden="true" />
              {link.label}
            </a>
          </li>
        ))}
      </ul>
    </div>
  )
}

/** Near-white footer with brand, link columns, contact info and Component Dock credit. */
export function Footer() {
  const year = new Date().getFullYear()
  const phoneHref = 'tel:' + footerContact.phone.replace(/[^\d+]/g, '')

  return (
    <footer id="contact" className="bg-black/[0.02] pt-28 pb-10">
      <div className="mx-auto grid max-w-[1250px] gap-10 px-4 md:grid-cols-2 lg:grid-cols-5">
        <div>
          <p className="mb-6 text-2xl font-semibold tracking-tight text-ink">
            {brandName}
            <span className="text-gold">.</span>
          </p>
          <p className="mb-6 text-body">{brandBlurb}</p>
          <ul className="flex gap-3">
            {socialLinks.map((social) => (
              <li key={social.label}>
                <a
                  href={social.href}
                  aria-label={social.label}
                  target="_blank"
                  rel="noreferrer"
                  className="flex h-[50px] w-[50px] items-center justify-center rounded-full bg-black/5 text-ink transition-colors hover:bg-brand hover:text-white"
                >
                  <BrandIcon label={social.label} />
                </a>
              </li>
            ))}
          </ul>
        </div>

        <FooterLinkColumn title="Community" links={communityLinks} />
        <FooterLinkColumn title="About Us" links={aboutUsLinks} />
        <FooterLinkColumn title="Company" links={companyLinks} />

        <div>
          <h2 className="mb-10 text-xl font-normal text-ink">Have a Questions?</h2>
          <ul className="space-y-4 text-black/80">
            <li className="flex gap-3">
              <MapPin className="mt-1 h-5 w-5 shrink-0 text-brand" aria-hidden="true" />
              <span>{footerContact.address}</span>
            </li>
            <li>
              <a
                href={phoneHref}
                className="flex items-center gap-3 transition-colors hover:text-brand"
              >
                <Phone className="h-5 w-5 shrink-0 text-brand" aria-hidden="true" />
                {footerContact.phone}
              </a>
            </li>
            <li>
              <a
                href={`mailto:${footerContact.email}`}
                className="flex items-center gap-3 transition-colors hover:text-brand"
              >
                <Mail className="h-5 w-5 shrink-0 text-brand" aria-hidden="true" />
                {footerContact.email}
              </a>
            </li>
          </ul>
        </div>
      </div>

      <div className="mt-16 border-t border-black/10 pt-6">
        <p className="mx-auto max-w-[1250px] px-4 text-center text-sm text-black/60">
          © {year} Homestead — All rights reserved | This template is made with{' '}
          <span aria-hidden="true">❤</span> by{' '}
          <a
            href="https://www.componentdock.com/"
            target="_blank"
            rel="noreferrer"
            className="font-semibold text-brand hover:underline"
          >
            Component Dock
          </a>
        </p>
      </div>
    </footer>
  )
}
