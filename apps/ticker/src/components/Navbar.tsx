...
export function Navbar() {
  ...
  const [dark, setDark] = useState(() => window.localStorage.getItem('ticker-theme') !== 'light')

  useEffect(() => {
    document.documentElement.classList.toggle('dark', dark)
    window.localStorage.setItem('ticker-theme', dark ? 'dark' : 'light')
    return () => {
    ...
    >
      <div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-4 py-4 sm:px-6 lg:px-8">
        ...
        <div className="hidden items-center gap-1 lg:flex">
          {links.map((link) => (
            <a
              key={link.label}
              href={link.href}
              className="rounded-lg px-4 py-2 text-sm font-medium text-gray-600 transition-colors hover:text-gray-900 dark:text-gray-300 dark:hover:text-white"
            >
              {link.label}
            </a>
          ))}
        ...
        <div className="flex items-center gap-3">
          ...
          <a
            href="#pricing"
            className="hidden text-sm font-medium text-gray-600 transition-colors hover:text-gray-900 sm:block dark:text-gray-300 dark:hover:text-white"
          >
            Sign in
          </a>
          ...
          <ButtonLink
            href="#pricing"
            className="bg-gradient-to-br from-primary-500 to-primary-600 px-5 py-2.5 text-sm font-semibold text-white"
          >
            Start Trading
          </ButtonLink>
      ...
      >
        <div className="flex flex-col gap-1 pt-2">
          {links.map((link) => (
            <a
              ...
              className="block rounded-lg px-3 py-2.5 text-sm font-medium text-gray-900 transition-colors hover:bg-gray-100 dark:text-white dark:hover:bg-gray-800"
            >
              {link.label}
            </a>
          ))}
        ...
        <div className="mt-3 flex flex-col gap-3">
          <a
            href="#pricing"
            onClick={() => setOpen(false)}
            className="block text-center text-sm font-medium text-gray-600 dark:text-gray-300"
          >
            Sign in
          </a>
          <ButtonLink
            href="#pricing"
            onClick={() => setOpen(false)}
            className="bg-gradient-to-br from-primary-500 to-primary-600 px-5 py-3 text-sm font-semibold text-white"
          >
            Start Trading
          </ButtonLink>
        </div>
