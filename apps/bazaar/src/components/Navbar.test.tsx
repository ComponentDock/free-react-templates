...
describe('Navbar', () => {
  ...
  it('shows the Bazaar wordmark and the primary links', () => {
    render(<Navbar />)

    expect(screen.getByRole('link', { name: /^Bazaar$/ })).toHaveAttribute('href', '#home')

    for (const label of ['Home', 'Shop', 'Catalogue', 'New Arrivals', 'Contact']) {
  ...
  it('toggles dark mode, persists it and cleans up on unmount', async () => {
    ...
    expect(window.localStorage.getItem('bazaar-dark')).toBe('1')

    await user.click(toggle)
    expect(document.documentElement.classList.contains('dark')).toBe(false)
    expect(window.localStorage.getItem('bazaar-dark')).toBe('0')
  })

  it('restores the persisted dark preference on load', () => {
    window.localStorage.setItem('bazaar-dark', '1')
    render(<Navbar />)
