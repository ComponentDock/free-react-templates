...
describe('Navbar', () => {
  it('renders the site name, nav links, CTA and dark-mode toggle', () => {
    render(<Navbar />)
    expect(screen.getByRole('link', { name: /Cortex/i })).toBeInTheDocument()
    for (const label of ['Platform', 'Pricing', 'Integrations', 'About', 'Blog', 'Contact']) {
  ...
  it('closes the mobile menu when a link or CTA is activated', async () => {
    ...
    await user.click(within(mobileNavAgain).getByRole('link', { name: 'Start Building' }))
    expect(screen.queryByRole('navigation', { name: 'Mobile navigation' })).not.toBeInTheDocument()
  })
})
