...
export function SocialIcons() {
  return (
    <div className="absolute bottom-7 left-8 flex items-center gap-2 md:left-15">
      <span className="mr-2 text-sm text-light">Stay in touch :</span>
      {socials.map(({ name, href, color, svg }) => (
        <a
          key={name}
          href={href}
          aria-label={name}
          className={`flex h-[35px] w-[35px] items-center justify-center rounded-full transition-colors hover:border ${color}`}
        >
          <svg
