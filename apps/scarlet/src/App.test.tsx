...
describe('App', () => {
  it('composes the navbar, error section, and footer and sets the document title', () => {
    ...
    expect(document.title).toBe('Scarlet — Error Page Template')
    ...
    expect(within(main).getByRole('img', { name: /background/i })).toBeInTheDocument()
    expect(within(main).getByRole('link', { name: 'Go Home' })).toHaveAttribute('href', '/')
