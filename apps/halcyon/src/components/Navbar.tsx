...
export function Navbar() {
  ...
  return (
    <header className="fixed inset-x-0 top-0 z-50 border-b border-white/10 bg-slate/95 backdrop-blur dark:bg-gray-950/95">
            ...
            {links.map((link) => (
              <a
                ...
                className={cn(
                  ...
                  link.label === 'Home' ? 'text-brand' : 'text-white/80',
                )}
              >
                {link.label}
              </a>
      ...
      {open && (
        <div className="fixed inset-0 z-50 flex flex-col bg-slate dark:bg-gray-950 lg:hidden">
          ...
          <nav aria-label="Mobile menu" className="flex flex-col gap-2 px-6 py-4">
            {links.map((link) => (
              <a
                ...
                onClick={() => setOpen(false)}
                className={cn(
                  'rounded-lg px-4 py-3 text-base font-semibold transition-colors hover:bg-white/10',
                  link.label === 'Home' ? 'text-brand' : 'text-white/80',
                )}
              >
                {link.label}
