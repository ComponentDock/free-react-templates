...
describe('Navbar', () => {
  it('renders the site name, links, phone number, and dark-mode toggle', () => {
    render(<Navbar />)
    expect(screen.getByRole('link', { name: /Girder/i })).toBeInTheDocument()
    expect(screen.getByRole('link', { name: /Home/i })).toBeInTheDocument()
    expect(screen.getByRole('link', { name: /Projects/i })).toBeInTheDocument()
    expect(screen.getByRole('link', { name: /Services/i })).toBeInTheDocument()
    expect(screen.getByRole('link', { name: /About/i })).toBeInTheDocument()
    expect(screen.getByRole('link', { name: /Contact/i })).toBeInTheDocument()
    expect(screen.getByText(/\+ 2 292 4392 327/i)).toBeInTheDocument()
  ...
  it('closes the mobile menu when a mobile link is clicked', async () => {
    ...
    const mobileLink = servicesLinks[servicesLinks.length - 1]!
