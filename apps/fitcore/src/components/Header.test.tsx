...
describe('Header', () => {
  it('shows the brand name and all nav links on desktop', () => {
    render(<Header />)
    expect(screen.getByRole('link', { name: /FitCore/i })).toHaveAttribute('href', '#home')
  ...
  it('closes the mobile menu when Become a Member is clicked', async () => {
    ...
    // The Become a Member link has text content, use getByText to distinguish from nav links
    const becomeMemberLinks = screen.getAllByText('Become a Member')
    // The last one is inside the mobile nav (first is desktop, second is mobile)
