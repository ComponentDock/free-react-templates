/** Pure validation + summary helpers for the Sleeply hotel-reservation form
    ...
    Destination. Guests and Children selects default to their first options
    (1 and 0). */
...
/** `2026-08-10` → `Aug 10` (no year) or `Aug 14, 2026` (with year), matching
    the summary format in the spec: "Downtown Dubai · 2 guests, 1 child ·
    Aug 10 – Aug 14, 2026". Parsed from the ISO parts so the local timezone
...
export function formatSummary(values: ReservationValues): string {
  ...
  return `${values.destination} · ${guestsLabel}, ${childrenLabel} · ${toDayLabel(values.checkIn, false)} – ${toDayLabel(values.checkOut, true)}`
}
