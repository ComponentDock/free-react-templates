...
export function Navbar() {
  ...
  const [dark, setDark] = useState(() => window.localStorage.getItem('sooth-theme') === 'dark')

  useEffect(() => {
    document.documentElement.classList.toggle('dark', dark)
    window.localStorage.setItem('sooth-theme', dark ? 'dark' : 'light')
    return () => {
      ...
      >
        ...
        <div className="flex items-center gap-4">
          ...
          <a
            href="#contact"
            className={cn(
              'hidden rounded-full bg-primary-600 px-5 py-2.5 text-sm font-medium text-white',
          ...
          <button
            ...
            className="rounded-full p-2 text-gray-500 transition-colors hover:bg-gray-100 dark:text-gray-400 dark:hover:bg-gray-800 lg:hidden"
          >
            {open ? (
