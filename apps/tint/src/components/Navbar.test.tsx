...
describe('Navbar', () => {
  it('renders the logo and desktop links', () => {
    render(<Navbar />)
    expect(screen.getByRole('link', { name: 'Tint' })).toHaveAttribute('href', '#home')
    for (const label of ['Home', 'About', 'Works', 'Services', 'Blog', 'Contact']) {
  ...
  it('closes the mobile menu when a mobile link is clicked', async () => {
    ...
    const worksLinks = screen.getAllByRole('link', { name: 'Works' })
    const mobileLink = worksLinks[worksLinks.length - 1]!
