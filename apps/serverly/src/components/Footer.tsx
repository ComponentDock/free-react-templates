import { Mail, MapPin, Phone } from 'lucide-react'

const columns = [
  {
    title: 'Hosting Packages',
    links: [
      'Cloud Hosting',
      'Web Hosting',
      'Reseller Hosting',
      'VPS Hosting',
      'Dedicated Servers',
      'Windows Hosting',
      'Linux Servers',
    ],
  },
  {
    title: 'Our Services',
    links: [
      'Web Design',
      'Logo Design',
      'Domains Register',
      'Search Advertising',
      'Email Marketing',
    ],
  },
  {
    title: 'Useful Links',
    links: ['About Us', 'Testimonials', 'Services', 'News', 'Contact'],
  },
] as const

const paymentCards = ['VISA', 'Mastercard', 'PayPal', 'Amex', 'Discover'] as const

const socials = [
  {
    name: 'Facebook',
    href: 'https://facebook.com',
    path: 'M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z',
  },
  {
    name: 'Twitter',
    href: 'https://twitter.com',
    path: 'M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z',
  },
  {
    name: 'Google',
    href: 'https://google.com',
    path: 'M12.48 10.92v3.28h7.84c-.24 1.84-.853 3.187-1.787 4.133-1.147 1.147-2.933 2.4-6.053 2.4-4.827 0-8.6-3.893-8.6-8.72s3.773-8.72 8.6-8.72c2.6 0 4.507 1.027 5.907 2.347l2.307-2.307C18.747 1.44 16.133 0 12.48 0 5.867 0 .307 5.387.307 12s5.56 12 12.173 12c3.573 0 6.267-1.173 8.373-3.36 2.16-2.16 2.84-5.213 2.84-7.667 0-.76-.053-1.467-.173-2.053H12.48z',
  },
  {
    name: 'Instagram',
    href: 'https://instagram.com',
    path: 'M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z',
  },
] as const

export function Footer() {
  const year = new Date().getFullYear()

  return (
    <footer id="contact" aria-label="Footer">
      <div className="bg-gradient-to-r from-brand-400 to-brand-500">
        <div className="mx-auto flex max-w-6xl flex-col items-center justify-between gap-4 px-4 py-8 sm:flex-row sm:px-6">
          <p className="text-lg font-semibold text-white">Need Help? Call Us 24/7</p>
          <a
            href="tel:652345322211"
            className="inline-flex items-center gap-3 text-4xl font-semibold text-white transition-opacity hover:opacity-90"
          >
            <Phone className="h-8 w-8" aria-hidden="true" />
            652-345 3222 11
          </a>
        </div>
      </div>

      <div className="bg-navy-800 pb-[51px] pt-[149px]">
        <div className="mx-auto grid max-w-6xl gap-12 px-4 sm:px-6 lg:grid-cols-5">
          <div className="lg:col-span-2">
            <a href="#home" className="flex items-baseline gap-1" aria-label="Serverly home">
              <span className="font-display text-[28px] font-bold tracking-wide text-white">
                <span className="uppercase text-royal-400">S</span>erverly
              </span>
            </a>
            <ul className="mt-6 space-y-3 text-sm text-mist-400">
              <li className="flex items-start gap-3">
                <MapPin className="mt-0.5 h-4 w-4 shrink-0 text-brand-400" aria-hidden="true" />
                <span>1481 Creekside Lane, Avila Beach, CA 931</span>
              </li>
              <li className="flex items-center gap-3">
                <Phone className="h-4 w-4 shrink-0 text-brand-400" aria-hidden="true" />
                <a href="tel:+53345795332453" className="transition-colors hover:text-white">
                  +53 345 7953 32453
                </a>
              </li>
              <li className="flex items-center gap-3">
                <Mail className="h-4 w-4 shrink-0 text-brand-400" aria-hidden="true" />
                <a href="mailto:yourmail@gmail.com" className="transition-colors hover:text-white">
                  yourmail@gmail.com
                </a>
              </li>
            </ul>
            <ul className="mt-6 flex flex-wrap gap-2" aria-label="Payment methods">
              {paymentCards.map((card) => (
                <li
                  key={card}
                  className="rounded border border-white/15 px-2 py-1 text-[10px] font-semibold tracking-wider text-white/70"
                >
                  {card}
                </li>
              ))}
            </ul>
            <ul className="mt-6 flex gap-3" aria-label="Social links">
              {socials.map((social) => (
                <li key={social.name}>
                  <a
                    href={social.href}
                    target="_blank"
                    rel="noreferrer"
                    aria-label={social.name}
                    className="flex h-9 w-9 items-center justify-center rounded-full bg-white/10 text-white transition-colors hover:bg-brand-400"
                  >
                    <svg
                      viewBox="0 0 24 24"
                      className="h-4 w-4"
                      fill="currentColor"
                      aria-hidden="true"
                    >
                      <path d={social.path} />
                    </svg>
                  </a>
                </li>
              ))}
            </ul>
          </div>

          {columns.map((column) => (
            <nav key={column.title} aria-label={column.title}>
              <h3 className="text-lg font-semibold text-white">{column.title}</h3>
              <ul className="mt-5 space-y-3">
                {column.links.map((link) => (
                  <li key={link}>
                    <a
                      href="#home"
                      className="text-sm text-mist-400 transition-colors hover:text-white"
                    >
                      {link}
                    </a>
                  </li>
                ))}
              </ul>
            </nav>
          ))}
        </div>
      </div>

      <div className="bg-navy-950 py-3">
        <p className="text-center text-xs text-white/70">
          © {year} Serverly. All rights reserved | This template is made with React &amp; Tailwind
          CSS.
        </p>
      </div>
    </footer>
  )
}
