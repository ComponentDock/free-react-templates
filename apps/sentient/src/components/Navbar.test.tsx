...
describe('Navbar', () => {
  ...
  it('shows the wordmark, section links, CTA and dark-mode toggle', () => {
    render(<Navbar />)
    expect(screen.getByRole('link', { name: /sentient home/i })).toBeInTheDocument()
    ...
    expect(within(nav).getByRole('link', { name: 'Try Free' })).toBeInTheDocument()
  ...
  it('reveals the mobile menu with the same links and CTA', async () => {
    ...
    expect(within(menu).getByRole('link', { name: 'Pricing' })).toBeInTheDocument()
    expect(screen.getAllByRole('link', { name: 'Try Free' })).toHaveLength(2)

    await user.click(within(menu).getByRole('link', { name: 'Pricing' }))
    expect(toggle).toHaveAttribute('aria-expanded', 'false')

    await user.click(toggle)
    await user.click(screen.getAllByRole('link', { name: 'Try Free' })[1]!)
    expect(toggle).toHaveAttribute('aria-expanded', 'false')
  })
  ...
  it('persists the dark preference to localStorage', async () => {
    ...
    render(<Navbar />)
    await user.click(screen.getByRole('button', { name: 'Toggle dark mode' }))
    expect(window.localStorage.getItem('sentient-dark')).toBe('1')
  })

  it('restores a persisted dark preference on mount', () => {
    window.localStorage.setItem('sentient-dark', '1')
    render(<Navbar />)
    expect(document.documentElement.classList.contains('dark')).toBe(true)
  })
})
