...
interface SocialLinksProps {
  /** 'plain' (top bar) or 'circle' (footer) presentation. */
...
export function SocialLinks({ variant = 'plain' }: SocialLinksProps) {
  ...
  return (
    <ul className={cn('flex', isCircle ? 'gap-2' : 'gap-1')}>
      {socials.map((social) => (
        <li key={social.name}>
          <a
            href={social.href}
            ...
            className={cn(
              ...
              isCircle
                ? 'h-9 w-9 rounded-full bg-muted text-white hover:bg-neutral-800'
                : 'h-8 w-8 text-gray-500 hover:text-primary-600',
            )}
          >
