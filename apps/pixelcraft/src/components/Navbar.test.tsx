...
describe('Navbar', () => {
  it('renders the logo and desktop links', () => {
    render(<Navbar />)
    expect(screen.getByRole('link', { name: 'Pixelcraft' })).toHaveAttribute('href', '#home')
    for (const label of ['Home', 'Services', 'Portfolio', 'About', 'Stories', 'Contact']) {
  ...
  it('closes the mobile menu when a mobile link is clicked', async () => {
    ...
    const mobileLink = portfolioLinks[portfolioLinks.length - 1]!
