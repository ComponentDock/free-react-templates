...
describe('Navbar', () => {
  it('renders the site name, top bar, and section links', () => {
    render(<Navbar />)
    expect(screen.getByRole('link', { name: /Motorly/i })).toBeInTheDocument()
    expect(screen.getByText(/Mon-Sat: 10:00 - 16:00/i)).toBeInTheDocument()
    expect(screen.getByRole('link', { name: /Find us on map/i })).toBeInTheDocument()
    for (const label of ['Home', 'Services', 'Shop', 'About', 'News', 'Contact']) {
